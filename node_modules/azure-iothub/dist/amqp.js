...
const azure_iot_amqp_base_1 = require("azure-iot-amqp-base");
const amqp_service_errors_js_1 = require("./amqp_service_errors.js");
const service_receiver_js_1 = require("./service_receiver.js");
...
function handleResult(errorMessage, done) {
    return (err, result) => {
        if (err) {
            ...
            done((0, amqp_service_errors_js_1.translateError)(errorMessage, err));
        }
        else {
...
function getTranslatedError(err, message) {
    ...
    return (0, amqp_service_errors_js_1.translateError)(message, err);
}
/**
...
class Amqp extends events_1.EventEmitter {
    /**
     * @private
     */
    constructor(config, amqpBase) {
        super();
        this._renewalNumberOfMilliseconds = 2700000;
        this._c2dEndpoint = '/messages/devicebound';
        this._feedbackEndpoint = '/messages/serviceBound/feedback';
        this._fileNotificationEndpoint = '/messages/serviceBound/filenotifications';
        this._bearerTokenPrefix = 'Bearer ';
        this._amqp = amqpBase ? amqpBase : new azure_iot_amqp_base_1.Amqp(true);
        this._config = config;
                        ...
                        this._amqp.initializeCBS((err) => {
                            if (err) {
                                ...
                                if (this._config.sharedAccessSignature) {
                                    const audience = azure_iot_common_1.SharedAccessSignature.parse(this._config.sharedAccessSignature.toString(), ['sr', 'sig', 'se']).sr;
                                    const isApplicationSuppliedSas = typeof (this._config.sharedAccessSignature) === 'string';
                                    const sasToken = isApplicationSuppliedSas ? this._config.sharedAccessSignature : this._config.sharedAccessSignature.extend((0, azure_iot_common_1.anHourFromNow)());
                        ...
                        if (this._feedbackReceiver) {
                            ...
                            this._amqp.attachReceiverLink(this._feedbackEndpoint, null, (err, link) => {
                                if (err) {
                                    ...
                                    this._feedbackReceiver = new service_receiver_js_1.ServiceReceiver(link);
                        ...
                        if (this._fileNotificationReceiver) {
                            ...
                            this._amqp.attachReceiverLink(this._fileNotificationEndpoint, null, (err, link) => {
                                if (err) {
                                    ...
                                    this._fileNotificationReceiver = new service_receiver_js_1.ServiceReceiver(link);
                                    ...
                                    if (err) {
                                        ...
                                        tmpC2DLink.detach((detachErr) => {
                                            if (detachErr) {
                                                ...
                                                if (!finalError) {
                                                    finalError = (0, amqp_service_errors_js_1.translateError)('error while detaching the c2d link when disconnecting', detachErr);
                                                }
                                            }
                                            else {
                                    ...
                                    if (err) {
                                        ...
                                        tmpFeedbackReceiver.detach((detachErr) => {
                                            ...
                                            if (!finalError && detachErr) {
                                                finalError = (0, amqp_service_errors_js_1.translateError)('error while detaching the message feedback link when disconnecting', detachErr);
                                            }
                                            callback();
                                        });
                                    }
                                ...
                                if (this._fileNotificationReceiver) {
                                    ...
                                    if (err) {
                                        ...
                                        tmpFileNotificationReceiver.detach((detachErr) => {
                                            ...
                                            tmpFileNotificationReceiver.removeListener('error', this._fileNotificationErrorListener);
                                            if (!finalError && detachErr) {
                                                finalError = (0, amqp_service_errors_js_1.translateError)('error while detaching the file upload notification link when disconnecting', detachErr);
                                            }
                                            callback();
                                        });
                                    }
                            ...
                            (callback) => {
                                this._amqp.disconnect((disconnectErr) => {
                                    ...
                                    if (!finalError && disconnectErr) {
                                        finalError = (0, amqp_service_errors_js_1.translateError)('error while disconnecting the AMQP connection', disconnectErr);
                                    }
                                    callback();
                                });
                            }
    ...
    connect(done) {
        return (0, azure_iot_common_1.callbackToPromise)((_callback) => {
            this._fsm.handle('connect', (err) => {
                if (err) {
                    _callback((0, amqp_service_errors_js_1.translateError)('AMQP Transport: Could not connect', err));
                }
                else {
    ...
    send(deviceId, message, done) {
        return (0, azure_iot_common_1.tripleValueCallbackToPromise)((_callback) => {
            const deviceEndpoint = azure_iot_common_1.endpoint.deviceMessagePath(encodeURIComponent(deviceId));
            ...
            const amqpMessage = azure_iot_amqp_base_1.AmqpMessage.fromMessage(message);
            this._fsm.handle('send', amqpMessage, deviceEndpoint, handleResult('AMQP Transport: Could not send message', _callback));
        }, (r, m) => {
            return (0, interfaces_js_1.createResultWithIncomingMessage)(r, m);
        }, done);
    }
    ...
    _handleSASRenewal() {
        const newSas = this._config.sharedAccessSignature.extend((0, azure_iot_common_1.anHourFromNow)());
